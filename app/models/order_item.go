...
type OrderItem struct {
	...
	// untuk tampilan gambar di template
	ProductImageURL string `gorm:"-"`
}
...
// dipakai di template: {{$item.BasePriceFloat}}
func (oi *OrderItem) BasePriceFloat() float64 {
	f, _ := oi.BasePrice.Float64()
	return f
}

// dipakai di template: {{$item.SubTotalFloat}}
func (oi *OrderItem) SubTotalFloat() float64 {
	f, _ := oi.SubTotal.Float64()
	return f
}

func (oi OrderItem) PriceFloat() float64 {
	f, _ := oi.BasePrice.Float64()
	return f
}
